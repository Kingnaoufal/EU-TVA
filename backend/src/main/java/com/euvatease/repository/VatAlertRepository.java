...
public interface VatAlertRepository extends JpaRepository<VatAlert, Long> {
    ...
    long countUnreadAlerts(@Nonnull @Param("shop") Shop shop);

    @Query("SELECT COUNT(a) FROM VatAlert a WHERE a.shop = :shop AND a.isResolved = false")
    long countUnresolvedAlerts(@Nonnull @Param("shop") Shop shop);

    boolean existsByShopAndAlertTypeAndIsResolvedFalse(@Nonnull Shop shop,
                                                       @Nonnull VatAlert.AlertType alertType);
    ...
    @Nonnull
    List<VatAlert> findByShopAndIsReadFalseOrderByCreatedAtDesc(@Nonnull Shop shop);

    @Nonnull
    List<VatAlert> findByShopAndIsResolvedFalseOrderByCreatedAtDesc(@Nonnull Shop shop);

    @Nonnull
    Page<VatAlert> findByShopOrderByCreatedAtDesc(@Nonnull Shop shop,
                                                  @Nonnull Pageable pageable);

    @Query("SELECT a FROM VatAlert a WHERE a.shop = :shop AND a.severity IN ('ERROR', 'CRITICAL') AND a.isResolved = false")
    @Nonnull
    List<VatAlert> findCriticalUnresolvedAlerts(@Nonnull @Param("shop") Shop shop);

    @Query("SELECT a FROM VatAlert a WHERE a.shop = :shop AND a.alertType = :type AND a.isResolved = false")
    @Nonnull
    List<VatAlert> findUnresolvedByType(@Nonnull @Param("shop") Shop shop,
                                        @Nonnull @Param("type") VatAlert.AlertType type);

    @Modifying
    @Query("UPDATE VatAlert a SET a.isRead = true WHERE a.shop = :shop AND a.isRead = false")
    int markAllAsRead(@Nonnull @Param("shop") Shop shop);
}
